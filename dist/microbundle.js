...
const createConfigItems = (type, items) => items.map(({
  name,
  ...options
}) => core.createConfigItem([require.resolve(name), options], {
  type
}));

var customBabel = babelPlugin.custom(babelCore => ({
  options({
  ...
  config(config, {
    ...
    if (envIdx !== -1) {
      ...
      babelOptions.presets[envIdx] = core.createConfigItem([preset.file.resolved, Object.assign(merge({
        loose: true,
        useBuiltIns: false,
        targets: customOptions.targets
      }, preset.options, {
        modules: false,
        exclude: merge(['transform-async-to-generator', 'transform-regenerator'], preset.options.exclude || [])
      }), customOptions.modern ? {
  ...
  try {
    return Promise.resolve(gzipSize(code)).then(function (_gzipSize) {
      ...
      return Promise.resolve(brotliSize(code)).then(function (_brotliSize) {
        const brotli = formatSize(_brotliSize, filename, 'br', raw || code.length < 5000);
        return gzip + '\n' + brotli;
      });
    });
  } catch (e) {
...
function createConfig(options, entry, format, writeMeta) {
  ...
  const moduleAliases = options.alias ? parseMappingArgument(options.alias) : {};
  ...
  let moduleMain = replaceName(pkg.module && !pkg.module.match(/src\//) ? pkg.module : pkg['jsnext:main'] || 'x.mjs', mainNoExtension);
  let modernMain = replaceName(pkg.syntax && pkg.syntax.esmodules || pkg.esmodule || 'x.modern.mjs', mainNoExtension);
      ...
      plugins: [].concat(postcss({
        ...
        writeBundle(bundle) {
          config._sizeInfo = Promise.all(Object.values(bundle).map(({
            code,
            fileName
          }) => code ? getSizeInfo(code, fileName, options.raw) : false)).then(results => results.filter(Boolean).join('\n'));
        }
